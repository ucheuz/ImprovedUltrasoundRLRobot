...
//----------------------------------------------------------------------
bool Kinematics_3p3::setupModel()
{
    ...
    xmlParseSuccess &= XmlUtils::getElementFromDoc(doc, "Robot", robotEl, "version", "3.3");
    ...
    for (int s = 0; s < sections.length(); s++) {
        ...
        if (!xmlParseSuccess) {
            std::cerr << "Error in Kinematics_3p3::setupModel: failed to read kinematics parameters from xml file for section " << sections[s].toStdString() << "." << std::endl;
            return false;
        }

        if (_nSectionJoints[s] != 11) {
            std::cerr << "Error in Kinematics_3p3::setupModel: inverse kinenmatics expects 11 joints in each of 2 arms." << std::endl;
            return false;
        }
...
void Kinematics_3p3::calcForwardKinematics(bool recalcProbePoses)
{
...
void Kinematics_3p3::calcInverseKinematics(bool recalcProbePoses)
{
    if (!_validModel) {
        std::cerr << "Error in Kinematics_3p3::calcInverseKinematics: invalid model setup for Robot v3.3." << std::endl;
        return;
    }
    ...
    double neutralDir_L = 65.0;
    double neutralDir_R = -115.0;
    ...
    solveSurfaceAngleTilt(unitSurfaceNormal_L, thetaZ_L, thetaXY_L, neutralDir_L, probeSeparation, t_L);
    solveSurfaceAngleTilt(unitSurfaceNormal_R, thetaZ_R, thetaXY_R, neutralDir_R, probeSeparation, t_R);
...
// different.
//----------------------------------------------------------------------
void Kinematics_3p3::solveSurfaceAngleTilt(const Point3D& surfaceNormal, double thetaZ, double thetaXY, double neutralXY, Point3D probeSeparation, std::vector<double>& jointPositions)
{
    ...
    while (relativeThetaXY > 180.0)
        relativeThetaXY -= 360.0;
    if (relativeThetaXY >= -90.0 && relativeThetaXY <= 90.0)
        // Side condition is end effector pointing down surface, according to thetaXY
        //jointPositions[6] = 0.0;
        jointPositions[6] = (1.0 - wZ) * relativeThetaXY;
    else if (relativeThetaXY < -90.0 && relativeThetaXY >= -135.0)
        // Side condition is end effector pointing down surface at -90 degrees.
        ...
        jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * -90.0;
    else if (relativeThetaXY > 90.0 && relativeThetaXY <= 135.0)
        // Side condition is as above, at +90 degrees.
        //jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * (relativeThetaXY - 90.0);
        jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * 90.0;
    else {
        ...
        if (relativeThetaXY <= -135.0)
            ...
            jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * ((1.0 - wXY) * 0.0 + wXY * -90.0);
        else
            //jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * ((1.0 - wXY) * 0.0 + wXY * 45.0);
            jointPositions[6] = (1.0 - wC) * (1.0 - wZ) * ((1.0 - wXY) * 0.0 + wXY * 90.0);
    }
...
//----------------------------------------------------------------------
void Kinematics_3p3::solveTiltOrientation(unsigned int sectionIdx, std::vector<double>& jointPositions)
{
...
// probe
//----------------------------------------------------------------------
void Kinematics_3p3::solveVerticalTranslation(unsigned int sectionIdx, std::vector<double>& jointPositions)
{
    if (sectionIdx >= _nSections)
        return;
...
//----------------------------------------------------------------------
void Kinematics_3p3::solveGantryTranslation(std::vector<double>& jointPositions_L, std::vector<double>& jointPositions_R)
{
    ...
    jointPositions_L[0] = jointPositions_R[0] = -(yTarget - yNeutral) - 50.0;  // +ve joint movement is -Y direction
...
//----------------------------------------------------------------------
void Kinematics_3p3::solveArmHorizontalTranslation(unsigned int sectionIdx, std::vector<double>& jointPositions, double neutralDir, bool leftArm)
{
    if (sectionIdx >= _nSections)
        return;
